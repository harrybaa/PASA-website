...
*/
	...
	function field($name, $type = 'text', $value = '', $premium_check = 0, $default = '', $option = '') {
		...
		if($premium_check && !$premium) {
			return '<input type="button" name="" value="'.__( "Only available in Times Square Premium - Upgrade Now", "times_square" ).'" onClick="location.href=\'/wp-admin/themes.php?page=theme_settings_page&action=premium\';" class="button button-primary" />';	
		} else  if($type == 'text') {
			...
			$field .= '<input type="button" id="'.$name.'" value="'.__( "Select Media", "times_square" ).'" class="upload" />';
		} else if ($type == 'color') {
			$field = '<input type="text" id="'.$name.'" name="'.$name.'" value="'.__( "Select Media", "times_square" ).'" class="color" />';
	...
	$field_list = array(
					...
					'favicon' => array(
									'title' => __( 'Favicon', 'times-square' ), 
									'type' => 'file', 
									'description' => __( 'Select your favicon for site.', 'times-square' ), 
									'premium' => '0', 
									'tab' => ''
							),
